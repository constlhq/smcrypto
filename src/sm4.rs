...
impl<'a> CryptSM4CBC<'a> {
    ...
    pub fn encrypt_cbc_padding_none(&self, input_data: &[u8]) -> Vec<u8> {
        encrypt_cbc_padding_none(input_data, self.key, self.iv)
    }
    ...
    pub fn decrypt_cbc_no_padding(&self, input_data: &[u8]) -> Vec<u8> {
        decrypt_cbc_no_padding(input_data, self.key, self.iv)
    }
